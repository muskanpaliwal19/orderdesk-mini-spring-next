...
count=$(curl -s "$API_URL/orders" | grep -o '{' | wc -l)
if [ "$count" -ne 4 ]; then
...
if [ -z "$created_order_id" ]; then
    ...
    curl -v -X POST -H "Content-Type: application/json" -d "$create_payload" "$API_URL/orders"
    exit 1
fi
...
csv__content=$(curl -s -H "Accept: text/csv" "$API_URL/orders/export.csv")
